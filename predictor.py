import cv2
from model import CountRegressor, Resnet50FPN, VGG16FPN
...
def predictorModel(input_image, bbox_list):
    ...
    if bbox_list is None: # if no bounding box file is given, prompt the user for a set of bounding boxes
        ...
        rects1 = list()
        for singleList in bbox_list:
            x1 = int(float(singleList[0]))
            y1 = int(float(singleList[1]))
            x2 = int(float(singleList[2]))
            y2 = int(float(singleList[3]))
            rects1.append([y1, x1, y2, x2])
